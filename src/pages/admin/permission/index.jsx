...
export const Permissions = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await ApiService.callFetchPermission(
          `page=${pageInfo.page}&size=10&sort=module,asc`
        );
        if (res.data.statusCode === 200) {
          ...
          console.log("fetch all permission", res);
          setIsLoading(false);
        } else {
  ...
  const deletePermission = async (permissionId) => {
    try {
      const res = await ApiService.callDeletePermission(permissionId);
      if (res.data.statusCode === 200) {
        setPermissions(skills.filter((skill) => skill.id !== permissionId));
      } else {
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-center justify-between mb-5">
            <h1 className="text-xl font-semibold">Danh sách quyền</h1>
            <Link
                ...
                <tbody>
                  {permissions.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{item.apiPath}</td>
                      ...
                      <td className="px-6 py-4 text-right flex items-center gap-3">
                        <Link
                          to={`/admin/permission/edit/${item.id}`}
