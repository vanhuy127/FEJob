...
export const CreateRole = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await ApiService.callFetchPermission(
          `page=1&size=200&sort=module,asc`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      console.log("[create role] Form submitted successfully", formData);
      try {
        const res = await ApiService.callCreateRole(formData);
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-center justify-between mb-5">
            <h1 className="text-xl font-semibold">Thêm mới vai trò</h1>
            <Link
              to={`/admin/role`}
