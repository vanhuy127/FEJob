...
export const Companies = () => {
  ...
  const deleteCompany = async (companyId) => {
    try {
      const res = await ApiService.callDeleteCompany(companyId);
      if (res.data.statusCode === 200) {
                  ...
                  {companies.map((company, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {company.createdAt
                          ? convertDateTime(company.createdAt)
                          : "-"}
                      </td>
                      <td className="px-6 py-4">
                        {company.updatedAt
                          ? convertDateTime(company.updatedAt)
                          : "-"}
                      </td>
                      <td className="px-6 py-4 text-right flex items-center gap-3">
                        <Link
                          to={`/admin/company/edit/${company.id}`}
